...
export const updateWaterLevels = action({
  handler: async (ctx): Promise<{
    ...
    try {
      ...
      const summaryResponse = await fetch(`${BASE_URL}/StationRiverLevels/GetWLStationSummary`);
      if (!summaryResponse.ok) {
        throw new Error(`HTTP ${summaryResponse.status}: Failed to fetch water level summary`);
      }
      ...
      const totalDanger = districts.reduce((sum: number, d: any) => sum + d.dangerCount, 0);
      const totalWarning = districts.reduce((sum: number, d: any) => sum + d.warningCount, 0);
      const totalAlert = districts.reduce((sum: number, d: any) => sum + d.alertCount, 0);
      ...
      const summaryId: string = await ctx.runMutation(internal.waterLevelData.storeWaterLevelSummaryInternal, {
        districts,
        overallStatus,
        scrapedAt: timestamp,
      });
      ...
      for (const district of districts) {
        try {
          const districtResponse = await fetch(`${BASE_URL}/StationRiverLevels/GetWLAllStationData/${district.districtId}`);
          if (districtResponse.ok) {
            ...
            const stations = stationsData.map((station: any) => ({
              ...
              z2: station.z2,
              z3: station.z3
            })).filter((station: any) => station.currentWaterLevel !== -9999);
            
            const result = await ctx.runMutation(internal.waterLevelData.storeDistrictStationsInternal, {
              districtId: district.districtId,
      ...
      console.log(`✅ Automated scraping complete: ${districts.length} districts, ${totalStationsSaved} stations, Status: ${overallStatus}`);
...
function getAlertLevel(waterLevel: number, dangerLevel: number, warningLevel: number, alertLevel: number): number {
